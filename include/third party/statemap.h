...
public:
    ...
    int getMinIndex() const { return (_minIndex); };
...
private:
    ...
    public:
        ...
        StateEntry* getNext() { return (_next); };
...
public:
    ...
    int same(const FSMContext& fsm) const { return (this == &fsm); };

    // Returns the debug flag's current setting.
    bool getDebugFlag() { return (_debug_flag); };
    ...
    bool isInTransition() const { return (_state == NULL ? true : false); };

    // Returns the current transition's name.
    // Used only for debugging purposes.
    char* getTransition() { return (_transition); };
    ...
    // May be NULL.
    State* getPreviousState() { return (_previous_state); }
    ...
    // otherwise.
    bool isStateStackEmpty() const { return (_state_stack_depth == 0); }

    // Returns the state stack's depth.
    int getStateStackDepth() const { return (_state_stack_depth); }
    ...
    void emptyStateStack() { _state_stack_depth = 0; };
#else  // ! SMC_FIXED_STACK
       // Returns true if the state stack is empty and false
       // otherwise.
    bool isStateStackEmpty() const { return (_state_stack == NULL); }
    ...
    int getStateStackDepth() const
    {
        ...
        for (state_ptr = _state_stack, retval = 0; state_ptr != NULL; state_ptr = state_ptr->getNext(), ++retval)
            ;
